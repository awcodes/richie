...
export default Node.create({
    ...
    renderHTML({ HTMLAttributes }) {
        return ['richie-block', JSON.stringify(HTMLAttributes)];
    },

    addCommands() {
        return {
            insertBlock: (attributes) => {
                return ({ editor, chain, state, tr }) => {
                    ...
                    if (tr.getMeta('isUpdatingBlock')) {
                        insertAt = {from: $from.pos, to: $to.pos}
                    } else if (tr.getMeta('isInsertingBlock')) {
                        insertAt = $from.pos
                    }

                    return chain().insertContentAt(insertAt, {type: this.name, attrs: attributes})
                }
            },
        }
    },
    ...
    addNodeView() {
        return ({ node }) => {
            ...
            const controls = `
                <div x-data>
                    <div class="richie-block-controls">
                        <button type="button" x-on:click="handleBlockUpdate('${node.attrs.identifier}')">
            ...
            container.setAttribute('data-richie-block', 'true')
